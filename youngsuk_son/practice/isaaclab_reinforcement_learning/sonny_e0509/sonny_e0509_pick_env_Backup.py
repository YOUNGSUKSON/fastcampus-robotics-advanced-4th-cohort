...
class SysE0509PickEnvCfg(DirectRLEnvCfg):
    ...
    success_reward = +40.0
    z_oppose_reward_scale = +7.0
    short_axis_align_reward_scale = +6.0
    ...
    grasp_enter_gate_threshold = 0.70
    grip_gate_pose_center = 0.90
    grip_gate_pose_k = 20.0
    grip_gate_dist_center = 0.02
    grip_gate_dist_k = 120.0
...
class SysE0509PickEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: SysE0509PickEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        short_axis_local = (1.0, 0.0, 0.0) if self.cfg.object_xy_size[0] <= self.cfg.object_xy_size[1] else (0.0, 1.0, 0.0)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # gripper collision with non-object geometry (e.g., table) counts as failure
        gripper_collision_fail = self._has_non_object_collision(self.gripper_collision_body_ids)
        terminated = success | dropped | gripper_collision_fail
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        z_oppose_reward = z_opposition * pose_proximity
        ...
        short_axis_align_reward = short_axis_align * pose_proximity

        # approach reward is strengthened only when pose is aligned
        pose_align = 0.5 * (z_opposition + short_axis_align)
        ...
        rewards = (
            ...
            + self.cfg.z_oppose_reward_scale * z_oppose_reward
            + self.cfg.short_axis_align_reward_scale * short_axis_align_reward
        ...
        self.extras["log"].update(
            {
                ...
                "KPI/04_pose_z": (self.cfg.z_oppose_reward_scale * z_oppose_reward).mean(),
                "KPI/05_pose_y": (self.cfg.short_axis_align_reward_scale * short_axis_align_reward).mean(),
