...
class SysE0509PickEnvCfg(DirectRLEnvCfg):
    ...
    dist_reward_scale = +1.4
    lift_reward_scale = +150.0
    success_reward = +300.0
    axis_align_reward_scale = +3.0
    # proportional gripper-position tracking as alignment/distance become ready
    gripper_target_track_reward_scale = +22.0
    gripper_target_progress_reward_scale = +20.0
    gripper_target_pos_tolerance = 0.03
    gripper_target_progress_norm = 0.03
    # target mapping saturation: 0.75 means align_progress=0.75 already maps to 100% target close
    gripper_target_full_progress = 0.80
    grasp_enter_gate_threshold = 0.50
    grip_gate_pose_center = 0.84
    grip_gate_pose_k = 18.0
    grip_gate_dist_center = 0.07
    grip_gate_dist_k = 48.0
    ...
    # final grasp width margin from object short side
    gripper_width_target_ready_margin_m = -0.0100

    arm_action_penalty_scale = -0.002

    # distance reward is gated by pose quality
    dist_pose_gate_min = 0.10
    pose_reward_proximity_k = 3.5
    pose_reward_far_scale = 0.30

    collision_penalty_scale = -0.3
...
class SysE0509PickEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: SysE0509PickEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # final 100% target uses the object-size based grasp margin target
        self.gripper_width_target_final_m = self.object_short_size_m + float(self.cfg.gripper_width_target_ready_margin_m)
        ...
        final_target_ratio = torch.clamp(
            torch.tensor(
                (self.gripper_width_target_final_m - self.cfg.gripper_open_width_m) / width_span,
                dtype=torch.float32,
                device=self.device,
            ),
            0.0,
        ...
        self.gripper_target_final_pos = float(self.gripper_pos_open + final_target_ratio.item() * self.gripper_pos_span)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # continuous alignment/distance progress for gripper target
        ...
        align_progress = gate_pose * gate_dist
        grasp_ready_now = (align_progress > self.cfg.grasp_enter_gate_threshold).float()
        full_prog = max(self.cfg.gripper_target_full_progress, 1.0e-6)
        target_progress = torch.clamp(align_progress / full_prog, 0.0, 1.0)

        # single gripper objective: follow moving target from open -> final grasp target
        ...
        gripper_target_pos = self.gripper_pos_open + target_progress * (
            self.gripper_target_final_pos - self.gripper_pos_open
        )
        ...
        # flat reward inside tolerance band, smooth decay outside
        gripper_target_track_reward = torch.where(
            gripper_target_err <= target_tol,
            torch.ones_like(gripper_target_err),
            torch.exp(-torch.square((gripper_target_err - target_tol) / target_tol)),
        )
        ...
        gripper_target_progress_reward = torch.clamp(target_err_delta / target_prog_norm, min=-1.0, max=1.0)
        ...
        target_close_ratio = torch.clamp(
            (gripper_target_pos - self.gripper_pos_open) / self.gripper_pos_span, min=0.0, max=1.0
        )
        width_target_m = self.cfg.gripper_open_width_m + target_close_ratio * (
            self.cfg.gripper_close_width_m - self.cfg.gripper_open_width_m
        )
        ...
        self.extras["log"].update(
            {
                ...
                "KPI/06_grip_tgt": (self.cfg.gripper_target_track_reward_scale * gripper_target_track_reward).mean(),
                "KPI/07_grip_prog": (
                    self.cfg.gripper_target_progress_reward_scale * gripper_target_progress_reward
                ).mean(),
                "KPI/08_collision": (self.cfg.collision_penalty_scale * collision_count).mean(),
                "KPI/09_act_arm": arm_action_penalty_term.mean(),
                ...
                "Diag/width_ready": align_progress.mean(),
